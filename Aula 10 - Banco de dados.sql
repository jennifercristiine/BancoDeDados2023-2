...
/*1- Salarios max e min*/
SELECT MAX(salario) AS Maiorsalario, min(salario) AS Menorsalario
FROM funcionario;

/*2- Quantos funcionarios são analistas*/
SELECT  COUNT(*)
FROM funcionario
WHERE CARGO = "Analista";

/*3- Folha Salárial da empresa (Soma de todos salarios)*/
SELECT SUM(salario)
FROM funcionario;

/*4- Média de Salario de quem trabalha para o RH*/
...
SELECT FUNCIONARIO.IDDEPT, DEPARTAMENTO.NOME, count(*)
FROM DEPARTAMENTO INNER JOIN FUNCIONARIO
ON funcionario.iddept = departamento.iddepartamento
group by FUNCIONARIO.IDDEPT;

/*3 - Qual a média dos salarios dos cargos por departamento?*/

SELECT DEPARTAMENTO.NOME, funcionario.cargo, AVG(FUNCIONARIO.SALARIO)
FROM DEPARTAMENTO INNER JOIN FUNCIONARIO
ON funcionario.iddept = departamento.iddepartamento
group by FUNCIONARIO.IDDEPT, FUNCIONARIO.CARGO;

/*O que acontece se tentarmos agregar os dados por CPF*/
SELECT 
	CPF
FROM
	FUNCIONARIO
...
/*1. Liste somente os departamentos que tem mais do que 5 funcionários(UTILIZANDO HAVING)*/
SELECT 
	CARGO, COUNT(CARGO)
FROM
	FUNCIONARIO
...
/*2. Liste os cargos que tem média salarial maior do que 5000*/
SELECT 
	CARGO, avg(SALARIO)
FROM
	FUNCIONARIO
...
/*3. Qual o departamento que tem o maior salário?*/
SELECT DEPARTAMENTO.NOME, max(FUNCIONARIO.SALARIO) AS MAX_SALARIO
FROM DEPARTAMENTO INNER JOIN FUNCIONARIO
ON funcionario.iddept = departamento.iddepartamento
group by departamento.iddepartamento
ORDER BY MAX(FUNCIONARIO.SALARIO) DESC
LIMIT 1;
